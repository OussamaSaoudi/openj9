...
*/
		...
		case CFR_BC_invokestatic:
			...
			UDATA returnChar = (UDATA) getReturnTypeFromSignature(classfile->constantPool[classfile->constantPool[index].slot2].bytes, classfile->constantPool[classfile->constantPool[index].slot2].slot1, NULL);
			if (info->bytes[0] == '<') {
...
checkMethodStructure (J9PortLibrary * portLib, J9CfrClassFile * classfile, UDATA methodIndex, U_8 * instructionMap, J9CfrError * error, U_32 flags, I_32 *hasRET)
{
	...
	if (method->accessFlags & CFR_ACC_STATIC) {
		UDATA returnChar = (UDATA) getReturnTypeFromSignature(classfile->constantPool[method->descriptorIndex].bytes, classfile->constantPool[method->descriptorIndex].slot1, NULL);
